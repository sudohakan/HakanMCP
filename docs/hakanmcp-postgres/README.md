...
A Docker-based testing environment for the **RuVector PostgreSQL extension** from [sudohakan/ruvector-postgres](https://hub.docker.com/r/sudohakan/ruvector-postgres).
...
npx hakanmcp ruvector setup --output ./my-ruvector
...
docker exec ruvector-postgres psql -U claude -d hakanmcp -c "SELECT ruvector_version();"
...
## Available Tables (HakanMCP Schema)
...
SELECT * FROM hakanmcp.search_similar(
...
SELECT * FROM hakanmcp.search_patterns(
    '[...]'::ruvector(384),
...
SELECT * FROM hakanmcp.find_agents(
    '[...]'::ruvector(384),
    'coder',                  -- agent type
    5                         -- limit
);
...
-- Hyperbolic search
SELECT * FROM hakanmcp.hyperbolic_search(
    '[...]'::ruvector(384),
    10,                       -- limit
    -1.0                      -- curvature
);
...
SELECT hakanmcp.cosine_similarity(a, b);

-- L2 distance helper
SELECT hakanmcp.l2_distance(a, b);

-- Get RuVector info
SELECT * FROM hakanmcp.ruvector_info();
```
...
```bash
docker exec ruvector-postgres psql -U claude -d hakanmcp -c "
INSERT INTO hakanmcp.embeddings (content, embedding, metadata)
VALUES (
    'Example content',
...
WHERE schemaname = 'hakanmcp' AND indexdef LIKE '%hnsw%';
"
```
...
To migrate from HakanMCP's sql.js/JSON storage to RuVector PostgreSQL:

### Option 1: Use CLI Import Tool (Recommended)

```bash
# Export current memory to JSON
npx hakanmcp memory list --format json > memory-export.json

# Import directly to PostgreSQL
npx hakanmcp ruvector import --input memory-export.json

# Or generate SQL file first (dry-run)
npx hakanmcp ruvector import --input memory-export.json --output import.sql
docker exec -i ruvector-postgres psql -U claude -d hakanmcp < import.sql
```
...
cat memory-export.json | jq -r '.[] | "INSERT INTO hakanmcp.memory_entries (key, value, namespace, metadata) VALUES (\047\(.key)\047, \047\(.value | tostring)\047, \047\(.namespace // "default")\047, \047\(.metadata // {} | tojson)\047::jsonb);"' > import.sql
...
Embeddings can be generated via HakanMCP MCP and updated:

```sql
-- After generating embedding via MCP
UPDATE hakanmcp.memory_entries
...
HakanMCP provides CLI commands for RuVector PostgreSQL management:
...
npx hakanmcp ruvector setup

# Output to custom directory
npx hakanmcp ruvector setup --output /path/to/dir

# Print files to stdout (for inspection)
npx hakanmcp ruvector setup --print

# Force overwrite existing files
npx hakanmcp ruvector setup --force
```
...
npx hakanmcp ruvector import --input data.json --container my-postgres

# Verbose output
npx hakanmcp ruvector import --input data.json --verbose
```
...
npx hakanmcp ruvector status --verbose

# Run database migrations
npx hakanmcp ruvector migrate --up

# Run performance benchmarks
npx hakanmcp ruvector benchmark --vectors 10000

# Analyze and optimize
npx hakanmcp ruvector optimize --analyze

# Backup data
npx hakanmcp ruvector backup --output backup.sql
```
...
- [RuVector PostgreSQL Docker Hub](https://hub.docker.com/r/sudohakan/ruvector-postgres)
- [HakanMCP Documentation](https://github.com/sudohakan/HakanMCP)
