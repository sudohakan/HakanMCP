...
-- Optimized for HakanMCP V3 Neural Features
--
...
CREATE OR REPLACE FUNCTION hakanmcp.attention_scores(
...
CREATE OR REPLACE FUNCTION hakanmcp.self_attention(
    source_ids UUID[],
...
BEGIN
    ...
    all_pairs AS (
        ...
        CROSS JOIN hakanmcp.embeddings t
...
CREATE OR REPLACE FUNCTION hakanmcp.cross_attention(
    query_ids UUID[],
...
CREATE OR REPLACE FUNCTION hakanmcp.multihead_attention_aggregate(
    query_embedding vector(384),
...
CREATE OR REPLACE FUNCTION hakanmcp.flash_attention_search(
    query_embedding vector(384),
...
CREATE OR REPLACE FUNCTION hakanmcp.attention_weighted_retrieve(
    query_embedding vector(384),
...
CREATE OR REPLACE FUNCTION hakanmcp.causal_attention(
    source_ids UUID[],
    temperature FLOAT DEFAULT 1.0
)
RETURNS TABLE (
...
BEGIN
    ...
    causal_pairs AS (
        ...
        FROM sources s
        JOIN hakanmcp.embeddings t ON t.created_at <= s.created_at  -- Causal mask
        WHERE t.embedding IS NOT NULL
    ),
    softmax_scores AS (
        SELECT
...
CREATE OR REPLACE FUNCTION hakanmcp.attention_matrix(
    embedding_ids UUID[],
    temperature FLOAT DEFAULT 1.0
)
RETURNS TABLE (
