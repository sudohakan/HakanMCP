...
-- Optimized for HakanMCP V3 HNSW Integration
--
...
DROP INDEX IF EXISTS hakanmcp.idx_embeddings_hnsw_optimized;
...
CREATE OR REPLACE FUNCTION hakanmcp.semantic_search(
...
CREATE OR REPLACE FUNCTION hakanmcp.rag_retrieve(
    query_embedding vector(384),
...
CREATE OR REPLACE FUNCTION hakanmcp.hybrid_search(
    query_embedding vector(384),
    keyword_query TEXT,
    limit_count INT DEFAULT 10,
...
CREATE OR REPLACE FUNCTION hakanmcp.cluster_search(
...
CREATE OR REPLACE FUNCTION hakanmcp.temporal_search(
    query_embedding vector(384),
    limit_count INT DEFAULT 10,
...
CREATE OR REPLACE FUNCTION hakanmcp.mmr_search(
    query_embedding vector(384),
    limit_count INT DEFAULT 10,
...
SELECT
    id,
    content,
    1 - (embedding <=> (SELECT embedding FROM hakanmcp.embeddings LIMIT 1)) AS similarity
FROM hakanmcp.embeddings
WHERE embedding IS NOT NULL
ORDER BY embedding <=> (SELECT embedding FROM hakanmcp.embeddings LIMIT 1)
LIMIT 10;
