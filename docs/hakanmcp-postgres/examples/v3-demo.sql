...
-- optimized for HakanMCP V3 features:
--
...
SET search_path TO hakanmcp, public;
...
CREATE TABLE IF NOT EXISTS hakanmcp.trajectories (
...
CREATE TABLE IF NOT EXISTS hakanmcp.reasoning_patterns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hakanmcp.hyperbolic_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    parent_id UUID REFERENCES hakanmcp.hyperbolic_embeddings(id),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    metadata JSONB DEFAULT '{}'
);
...
ON hakanmcp.reasoning_patterns
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 100);

CREATE INDEX IF NOT EXISTS idx_hyperbolic_hnsw
ON hakanmcp.hyperbolic_embeddings
...
CREATE OR REPLACE FUNCTION hakanmcp.trajectory_start(
    p_task TEXT,
...
BEGIN
    ...
    INSERT INTO hakanmcp.trajectories (trajectory_id, agent_type, task_description)
...
CREATE OR REPLACE FUNCTION hakanmcp.trajectory_step(
...
CREATE OR REPLACE FUNCTION hakanmcp.trajectory_end(
    p_trajectory_id TEXT,
    p_success BOOLEAN,
...
BEGIN
    ...
    UPDATE hakanmcp.trajectories t
    SET
        status = 'completed',
        outcome = v_outcome,
        ended_at = NOW(),
...
CREATE OR REPLACE FUNCTION hakanmcp.pattern_store(
    p_pattern_id TEXT,
    p_description TEXT,
...
DECLARE
    v_id UUID;
BEGIN
    INSERT INTO hakanmcp.reasoning_patterns (
...
CREATE OR REPLACE FUNCTION hakanmcp.pattern_search(
...
BEGIN
    ...
    SELECT
        ...
        END AS success_rate,
        rp.pattern_type
    FROM hakanmcp.reasoning_patterns rp
...
CREATE OR REPLACE FUNCTION hakanmcp.pattern_feedback(
    p_pattern_id TEXT,
    p_success BOOLEAN,
    p_quality FLOAT DEFAULT 0.5
)
...
BEGIN
    ...
    FROM hakanmcp.reasoning_patterns
    WHERE pattern_id = p_pattern_id;

    -- Update with EWC++-inspired update rule
    UPDATE hakanmcp.reasoning_patterns
    SET
...
CREATE OR REPLACE FUNCTION hakanmcp.euclidean_to_poincare(
...
CREATE OR REPLACE FUNCTION hakanmcp.poincare_distance(
    p_x vector(384),
    p_y vector(384),
    p_curvature FLOAT DEFAULT -1.0
)
...
CREATE OR REPLACE FUNCTION hakanmcp.hyperbolic_search(
    p_query_embedding vector(384),
...
BEGIN
    ...
    v_query_poincare := hakanmcp.euclidean_to_poincare(p_query_embedding, p_curvature);
    ...
    SELECT
        ...
        hakanmcp.poincare_distance(he.poincare_embedding, v_query_poincare, p_curvature) AS poinc_dist,
        he.hierarchy_level,
        he.metadata
    FROM hakanmcp.hyperbolic_embeddings he
...
CREATE OR REPLACE FUNCTION hakanmcp.share_agent_memory(
    p_source_agent TEXT,
...
BEGIN
    ...
    FOREACH v_target IN ARRAY p_target_agents LOOP
        ...
        UPDATE hakanmcp.agents
        SET
            memory_embedding = (
                SELECT AVG(embedding)::vector(384)
                FROM (
                    SELECT memory_embedding AS embedding FROM hakanmcp.agents WHERE agent_id = v_target
                    UNION ALL
...
CREATE OR REPLACE FUNCTION hakanmcp.find_complementary_agents(
...
INSERT INTO hakanmcp.embeddings (content, metadata) VALUES
...
INSERT INTO hakanmcp.agents (agent_id, agent_type, state) VALUES
...
INSERT INTO hakanmcp.reasoning_patterns (pattern_id, pattern_type, description, confidence) VALUES
...
-- Count records
SELECT 'embeddings' AS table_name, COUNT(*) AS count FROM hakanmcp.embeddings
UNION ALL
SELECT 'patterns', COUNT(*) FROM hakanmcp.patterns
UNION ALL
SELECT 'agents', COUNT(*) FROM hakanmcp.agents
UNION ALL
SELECT 'reasoning_patterns', COUNT(*) FROM hakanmcp.reasoning_patterns
UNION ALL
SELECT 'trajectories', COUNT(*) FROM hakanmcp.trajectories;

-- Show indices
SELECT indexname, indexdef
FROM pg_indexes
WHERE schemaname = 'hakanmcp'
...
\echo '  - hakanmcp.semantic_search(embedding, limit, threshold)'
\echo '  - hakanmcp.rag_retrieve(embedding, coarse_limit, final_limit)'
\echo '  - hakanmcp.hybrid_search(embedding, keyword, limit)'
\echo '  - hakanmcp.trajectory_start(task, agent_type)'
\echo '  - hakanmcp.trajectory_step(id, action, result, quality)'
\echo '  - hakanmcp.trajectory_end(id, success, feedback)'
\echo '  - hakanmcp.pattern_store(id, desc, embedding, type)'
\echo '  - hakanmcp.pattern_search(embedding, limit, confidence)'
\echo '  - hakanmcp.pattern_feedback(id, success, quality)'
\echo '  - hakanmcp.hyperbolic_search(embedding, limit, curvature)'
\echo '  - hakanmcp.share_agent_memory(source, targets, filter)'
\echo '  - hakanmcp.find_complementary_agents(task_embedding)'
\echo ''
