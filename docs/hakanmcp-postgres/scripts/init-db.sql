...
-- from sudohakan/ruvector-postgres with HakanMCP V3 integration.
--
...
-- Create the hakanmcp schema
CREATE SCHEMA IF NOT EXISTS hakanmcp;

-- Grant permissions
GRANT ALL ON SCHEMA hakanmcp TO claude;
...
CREATE TABLE IF NOT EXISTS hakanmcp.embeddings (
...
CREATE TABLE IF NOT EXISTS hakanmcp.patterns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hakanmcp.agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hakanmcp.graph_nodes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS hakanmcp.graph_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_id UUID REFERENCES hakanmcp.graph_nodes(id),
    target_id UUID REFERENCES hakanmcp.graph_nodes(id),
...
CREATE OR REPLACE FUNCTION hakanmcp.search_similar(
...
CREATE OR REPLACE FUNCTION hakanmcp.search_patterns(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION hakanmcp.find_agents(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION hakanmcp.to_poincare(
    euclidean real[],
    curvature FLOAT DEFAULT -1.0
)
RETURNS real[] AS $$
...
BEGIN
    ...
    query_poincare := hakanmcp.to_poincare(query_arr, curvature);
...
CREATE OR REPLACE FUNCTION hakanmcp.ruvector_info()
RETURNS TABLE (
    version TEXT,
    simd_info TEXT
) AS $$
...
CREATE OR REPLACE FUNCTION hakanmcp.cosine_similarity(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- L2 distance helper
CREATE OR REPLACE FUNCTION hakanmcp.l2_distance(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
CREATE OR REPLACE FUNCTION hakanmcp.update_embedding(
    embedding_id UUID,
...
INSERT INTO hakanmcp.patterns (name, pattern_type, description, confidence) VALUES
...
BEGIN
    ...
    RAISE NOTICE '  - hakanmcp.search_similar(embedding, limit, min_sim)';
    RAISE NOTICE '  - hakanmcp.search_patterns(embedding, type, limit)';
    RAISE NOTICE '  - hakanmcp.find_agents(embedding, type, limit)';
    RAISE NOTICE '  - hakanmcp.hyperbolic_search(embedding, limit, curvature)';
    RAISE NOTICE '  - hakanmcp.cosine_similarity(a, b)';
    RAISE NOTICE '';
