...
run_timed "Simple similarity search" \
    "SELECT id FROM hakanmcp.embeddings WHERE embedding IS NOT NULL ORDER BY embedding <-> ruvector.fastembed('test query', 'all-MiniLM-L6-v2') LIMIT 10"

run_timed "Search with threshold" \
    "SELECT id, content FROM hakanmcp.embeddings WHERE embedding IS NOT NULL AND ruvector.cosine_similarity(embedding, ruvector.fastembed('test', 'all-MiniLM-L6-v2')) > 0.5 ORDER BY embedding <-> ruvector.fastembed('test', 'all-MiniLM-L6-v2') LIMIT 10"
...
PGPASSWORD=$PASSWORD psql -h $HOST -p $PORT -U $USER -d $DATABASE -c \
    "DELETE FROM hakanmcp.embeddings WHERE metadata->>'benchmark' = 'true'" > /dev/null 2>&1
...
for ((i=1; i<=ITERATIONS; i++)); do
    PGPASSWORD=$PASSWORD psql -h $HOST -p $PORT -U $USER -d $DATABASE -t -c \
        "INSERT INTO hakanmcp.embeddings (content, embedding, metadata) VALUES ('Benchmark content $i', ruvector.fastembed('Benchmark content $i', 'all-MiniLM-L6-v2'), '{\"benchmark\": true}'::jsonb)" > /dev/null 2>&1
done
