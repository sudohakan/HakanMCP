...
const config = {
  ...
  database: process.env.PGDATABASE || 'hakanmcp',
  user: process.env.PGUSER || 'claude',
  password: process.env.PGPASSWORD || 'hakanmcp-test',
};
...
async function testSchemaExists(client: PoolClient): Promise<void> {
  const result = await client.query(`
    ...
    WHERE schema_name = 'hakanmcp'
  `);
  if (result.rows.length === 0) {
    throw new Error('hakanmcp schema not found');
  }
}
...
async function testTablesExist(client: PoolClient): Promise<void> {
  ...
  for (const table of tables) {
    const result = await client.query(`
      ...
      WHERE table_schema = 'hakanmcp' AND table_name = $1
    `, [table]);
    if (result.rows.length === 0) {
...
async function testSearchLatency(client: PoolClient): Promise<void> {
  // Warm up
  await client.query(`
    SELECT id FROM hakanmcp.embeddings
    WHERE embedding IS NOT NULL
