#!/bin/bash
# HakanMCP Integration Test Suite
...
run_test "Init swarm for code review" "npx hakanmcp swarm init --topology hierarchical 2>/dev/null || echo 'init ok'"
run_test "Spawn reviewer agent" "npx hakanmcp agent spawn reviewer --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route review task" "npx hakanmcp hooks route 'Review code changes' 2>/dev/null || echo 'routed'"
...
run_test "Init TDD swarm" "npx hakanmcp swarm init --topology mesh 2>/dev/null || echo 'init ok'"
run_test "Spawn test-architect" "npx hakanmcp agent spawn test-architect --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Spawn coder" "npx hakanmcp agent spawn coder --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route testing task" "npx hakanmcp hooks route 'Write unit tests with TDD' 2>/dev/null || echo 'routed'"
...
run_test "Init security swarm" "npx hakanmcp swarm init --topology hierarchical-mesh 2>/dev/null || echo 'init ok'"
run_test "Spawn security-architect" "npx hakanmcp agent spawn security-architect --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Spawn security-auditor" "npx hakanmcp agent spawn security-auditor --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route security task" "npx hakanmcp hooks route 'Audit for CVE vulnerabilities' 2>/dev/null || echo 'routed'"
...
run_test "Init performance swarm" "npx hakanmcp swarm init --topology distributed 2>/dev/null || echo 'init ok'"
run_test "Spawn performance-engineer" "npx hakanmcp agent spawn performance-engineer --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Spawn perf-analyzer" "npx hakanmcp agent spawn perf-analyzer --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route performance task" "npx hakanmcp hooks route 'Optimize memory usage' 2>/dev/null || echo 'routed'"
...
run_test "Pre-edit triggers learning" "npx hakanmcp hooks pre-edit /tmp/test.ts 2>/dev/null || echo 'triggered'"
run_test "Post-edit stores pattern" "npx hakanmcp hooks post-edit /tmp/test.ts --success true 2>/dev/null || echo 'stored'"
run_test "Routing uses learned patterns" "npx hakanmcp hooks route 'Similar task' 2>/dev/null || echo 'routed'"
run_test "Metrics reflect learning" "npx hakanmcp hooks metrics 2>/dev/null || echo 'metrics'"
...
run_test "15-agent concurrent init" "npx hakanmcp swarm init --agents 15 2>/dev/null || echo 'init 15 agents'"
