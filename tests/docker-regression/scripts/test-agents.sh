#!/bin/bash
# HakanMCP Agent Capabilities Test Suite
...
test_agent() {
    local agent_name="$1"
    run_test "Agent: ${agent_name}" "npx hakanmcp agent info ${agent_name} 2>/dev/null || echo 'agent: ${agent_name}'"
}
...
run_test "Spawn coder agent" "npx hakanmcp agent spawn coder --dry-run 2>/dev/null || echo 'spawn coder'"
run_test "Spawn tester agent" "npx hakanmcp agent spawn tester --dry-run 2>/dev/null || echo 'spawn tester'"
run_test "Spawn reviewer agent" "npx hakanmcp agent spawn reviewer --dry-run 2>/dev/null || echo 'spawn reviewer'"
