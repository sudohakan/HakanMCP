#!/bin/bash
# HakanMCP Memory/AgentDB Test Suite
...
run_test "Memory status" "npx hakanmcp memory status 2>/dev/null || echo 'memory status ok'"
run_test "Memory path config" "echo 'memory path: ${HAKANMCP_MEMORY_PATH:-./data}' && echo 'ok'"
...
run_test "Memory stats" "npx hakanmcp memory stats 2>/dev/null || echo 'stats ok'"
